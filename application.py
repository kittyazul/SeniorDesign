...
application.config['SECRET_KEY'] = 'phishme'
application.config['SAML_PATH'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'saml')


def init_saml_auth(req):
    auth = OneLogin_Saml2_Auth(req, custom_base_path=application.config['SAML_PATH'])
    return auth
...
def index():
    ...
    if 'sso' in request.args:
        ...
        return redirect(auth.logout(name_id=name_id,
            session_index=session_index, nq=name_id_nq,
            name_id_format=name_id_format, spnq=name_id_spnq,
            return_to='http://uc.phishingguard.net'))
    ...
    if 'samlUserdata' in session:
       ...
       # return_to = '%sscan' % request.host_url
       # return redirect(auth.login(return_to))
...
def login():
    return redirect(request.host_url + '?sso')

@application.route('/attrs/')
def attrs():
    paint_logout = False
    attributes = False
